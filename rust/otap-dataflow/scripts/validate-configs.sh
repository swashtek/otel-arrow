...
for config in $CONFIG_FILES; do
    ...
    # Check for a platform constraint comment at the top of the file, e.g.:
    #   # platform: windows-only
    # Configs without the marker are platform-agnostic and always validated.
    ...
    case "$required_platform" in
        ...
        *)
            echo "  ❌ $REL_PATH (invalid platform marker: $required_platform)"
            FAILED=$((FAILED+1))
